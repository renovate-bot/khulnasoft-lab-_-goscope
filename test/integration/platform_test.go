	...
	*/
	...
	for _, tt := range tests {
		...
		t.Run(fmt.Sprintf("%s/%s", tt.source.String(), platform), func(t *testing.T) {
			...
			platformOpt := goscope.WithPlatform(platform)
			img, err := goscope.GetImageFromSource(context.TODO(), imageName, tt.source, platformOpt)
...
func TestDigestThatNarrowsToOnePlatform(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			img, err := goscope.GetImageFromSource(context.TODO(), imageStrWithDigest, tt.source)
...
func TestDefaultPlatformWithOciRegistry(t *testing.T) {
	img, err := goscope.GetImageFromSource(context.TODO(), "busybox:1.31", image.OciRegistrySource)
